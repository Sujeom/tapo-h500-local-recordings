...
- **One notification per visitor:** `tapo_h500_visit` fires once when somebody
  ...
  Two doorbells watching the same path still fire once between them. A third
  [blueprint](blueprints/automation/tapo_h500/announce_a_visit.yaml) is built
  on it, with a strangers-only filter and an after-dark gate.
